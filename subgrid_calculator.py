            ...
            # for ele in range(2):
                ...
                for j in range(3):
                    ...
                    if(with1D):
                        ...
                        # get bottom friction
                        cf[ele,j,:] = np.sum(tempcf,axis=1)/wetCellsInSubAreaCount # this is correct I need <Cf>W
                        # cf[ele,j,:] = np.nansum(tempcf,axis=1)/cellsInSubElement
                        
                        # get rv for advection correction and bottom friction correction
                        
                        rv[ele,j,:] = wetTotWatDepth[ele,j,:]/(np.sum((temptotWatDepth**(3/2))*(tempcf**(-1/2)),axis=1)/wetCellsInSubAreaCount)
                        
                        # get advection correction
                        
                        cadv[ele,j,:] = (1/wetTotWatDepth[ele,j,:])*(np.sum(temptotWatDepth**2/tempcf,axis=1)/wetCellsInSubAreaCount)*rv[ele,j,:]**2
            ...
            # for ele in range(2):
                ...
                for j in range(3):
                    ...
                    if(with1D):
                        ...
                        maxElevationSubEle[ele,j] = cp.max(bathyTopoInsideSubElement)
                        ...
                        # get bottom friction
                        cupycf = cp.sum(tempcf,axis=1)/wetCellsInSubAreaCount # this is correct I need <Cf>W
                        # cupycf = cp.nansum(tempcf,axis=1)/cellsInSubElement # this is incorrect
                        cf[ele,j,:] = cp.asnumpy(cupycf)
                        
                        # get rv for advection correction and bottom friction correction
                        cupyrv = cupywetTotWatDepth/(cp.sum((temptotWatDepth**(3/2))*(tempcf**(-1/2)),axis=1)/wetCellsInSubAreaCount)        
                        rv[ele,j,:] = cp.asnumpy(cupyrv)
                        
                        # get advection correction
                        cupyadv = (1/cupywetTotWatDepth)*(cp.sum(temptotWatDepth**2/tempcf,axis=1)/wetCellsInSubAreaCount)*cupyrv**2
                        cadv[ele,j,:] = cp.asnumpy(cupyadv)

...
class SubgridCalculatorDGP0():
    ...
    def calculateLookupTableForVectorizedStorage(self):
        ...
        for i in range(len(self.control.demFilenameList)):
            ...
            # for ele in elementList[0:100]:
                ...
                # get the min/max elevation of the element
                minElev = cp.ndarray.get(cp.min(zCutGeoTiffMatrix2masked))
                maxElev = cp.ndarray.get(cp.max(zCutGeoTiffMatrix2masked))
    ...
    ########## CALCULATE TOTAL WATER DEPTH BASED SUBGRID CORRECTION ##########
    ...
    def calculateTotWatDepthBasedLookupTable(self):
        ...
        surfElevs = xp.zeros((numEle,numLevels)).astype(xp.float32)
        ...
        surfElevs[xp.where(binaryElementList == 0),:] = -99999
        ...
        for i in range(len(self.control.demFilenameList)):
            ...
            # for ele in elementList[0:100]:
                ...
                ##############################################################
                # STEP1: surfaceElevation -> gridAvgTotWatDepth
                ##############################################################
                ...
                if xp.size(surfaceElevations) == numSurfaceElevLevels:
                    surfaceElevations[-1] = maxElev
                else:
                ...
                ############### STORE THE TABLE ###############################
                surfaceElevationsStep1 = surfaceElevations
                gridAvgTotWatDepthStep1 = gridAvgTotWatDepth
                surfaceElevations = []
                gridAvgTotWatDepth = []

                ##############################################################
                # STEP2: gridAvgTotWatDepth -> surfaceElevation and others
                ##############################################################

                ############ FIND THE TOTAL DEPTH LIST WITH CONSTANT INTERVAL ############
                minDepth = xp.min(gridAvgTotWatDepthStep1)
                maxDepth = xp.max(gridAvgTotWatDepthStep1)
                totWatDepthIntvl = (maxDepth - minDepth)/(numLevels - 1)
                gridAvgTotWatDepth = xp.arange(minDepth, maxDepth, totWatDepthIntvl)
                if xp.size(gridAvgTotWatDepth) == numLevels:
                    gridAvgTotWatDepth[-1] = maxDepth
                else:
                    gridAvgTotWatDepth = xp.append(gridAvgTotWatDepth,maxDepth)

                ############ FIND THE SURFACE ELEVATION LIST BASED ON THE TOTAL WATER DEPTH ############
                def findInverseTable(mapfr, mapto, inv_mapfr):
                    szinv_mapfr = inv_mapfr.size
                    inv_mapto = xp.zeros(szinv_mapfr)
                    ii = 0
                    inv_mapto[ii] = mapfr[ii]
                    jj = 0
                    for ii in range(1,szinv_mapfr-1):
                        vfr = inv_mapfr[ii]
                        found = False
                        while not found:
                            if vfr >= mapto[jj] and vfr < mapto[jj+1]:
                                s = (vfr - mapto[jj])/(mapto[jj+1]-mapto[jj])
                                inv_mapto[ii] = (1.0-s)*mapfr[jj] + s*mapfr[jj+1]
                                found = True
                            else:
                                jj += 1
                                if jj >= mapto.size-1:
                                    raise Exception("jj reached mapto.size-1")
                    inv_mapto[szinv_mapfr-1] = mapfr[-1]
                    
                    return inv_mapto

                surfaceElevations = findInverseTable\
                    (surfaceElevationsStep1, gridAvgTotWatDepthStep1, gridAvgTotWatDepth)
                ...
                totWatElementList = xp.sum(tempTotWatDepthWetArray,axis=0)
                # count how many cells are wet
                countWetInElement = xp.count_nonzero(tempWetDryList,axis=0)
                ...
                # finally sum and add cf for use in grid averaged calculation
                # tempcf = np.nansum(tempcf,axis=0)
                tempcf = xp.nansum(tempcf,axis=0)
                cfElementList = tempcf

                # Okay now we can finalize the values
                wetAvgTotWatDepth = totWatElementList/countWetInElement
                # gridAvgTotWatDepthForCheck = totWatElementList/countInElement
                wetFractionTemp = countWetInElement/countInElement
                cfTemp = cfElementList/countInElement
                cmfTemp = (wetAvgTotWatDepth)*(wetAvgTotWatDepth/(rvBottomTermList/countWetInElement))**2

                def get_ndarray(array):
                    if isinstance(array, cp.ndarray):
                        return xp.ndarray.get(array)
                    else:
                        return array
                    
                # get surface elevations
                surfElevs[ele,:] = get_ndarray(surfaceElevations)
                     
                # get grid averaged total water depth
                totWatDepth[ele,:] = get_ndarray(gridAvgTotWatDepth)
                maxTotDepthEle[ele] = xp.max(gridAvgTotWatDepth)

                # get wet area fraction for the subelement 
                wetFraction[ele,:] = get_ndarray(wetFractionTemp)

                # get grid averaged coefficient of friction for the element
                # convert back to numpy
                cf[ele,:] = get_ndarray(cfTemp)
                if self.level0andLevel1:
                    # get corrected bottom friction for level 1 corrections
                    cmf[ele,:] = get_ndarray(cmfTemp)
        ...
        # add bottom limit on cf and cmf
        cf[cf<0.0025] = 0.0025
        cf[xp.isnan(cf)] = 0.0025
        if self.level0andLevel1:
            cmf[cmf<0.0025] = 0.0025   
            cmf[xp.isnan(cmf)] = 0.0025

        self.subgriddb.totWatDepth = totWatDepth
        self.subgriddb.surfaceElevations = surfElevs
        self.subgriddb.wetFraction = wetFraction
        self.subgriddb.binaryElementList = binaryElementList
        self.subgriddb.minElevationEle = minElevationEle
        self.subgriddb.maxElevationEle = maxElevationEle
        self.subgriddb.maxTotDepthEle = maxTotDepthEle
        self.subgriddb.cfElement = cf
        if self.level0andLevel1:
            self.subgriddb.cmfElement = cmf
    ...
    def plot_subgrid_for_vectorizedstorage(self):
        import numpy as np
        from matplotlib.patches import Circle, Wedge, Polygon
        from matplotlib.collections import PatchCollection
        import matplotlib.pyplot as plt

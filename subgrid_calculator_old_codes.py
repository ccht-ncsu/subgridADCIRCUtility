                ...
                for j in range(3):
                    ...
                    for k in range(num_SfcElevs):
                        ...
                        if(wetCellsInSubAreaCount > 0):
                            ...
                            rv[ele,j,k] = totWatDepth[ele,j,k]/(np.nansum((temptotWatDepth**(3/2))*(tempcf**(-1/2)))/wetCellsInSubAreaCount) # fixed rv calc
        ...
        for i in range(len(demFilenameList)):
            ...
            # for ele in range(2):
                ...
                for j in range(3):
                    ...
                    # bathyTopoInsideSubElementFlat = bathyTopoInsideSubElement.flatten()
                    ...
                    # rv[ele,j,:] = totWatDepth[ele,j,:]/(np.nansum((temptotWatDepth**(3/2))*(tempcf**(-1/2)),axis=1)/wetCellsInSubAreaCount)
...
################## SUBGRID CPU CALULATOR V2 WITH PARALLEL ####################
# the purpose of this code was to hopefully speed up the original cpu code and
# reduce the memory usage as well as make it parallel --- fingers crossed!

    def calculateSubgridCorrectionv2parallel(controlFilename):
        ...
        # add number of threads I want to use
        e = ThreadPoolExecutor(4)
        # e = ProcessPoolExecutor(4)
        
        # make function out of subgrid calculator to use multiprocess
        
        def parallelDEM(demAndLC):
            
            # for i in range(len(demFilenameList)):
                
            # reading in DEM again
            # all variables the same as before
            elevationData = subgridCalculatormain.importDEMv2(demAndLC[0])
            landcoverData = subgridCalculatormain.importDEMv2(demAndLC[1])
            demNumber = demAndLC[2]
            ...
            # get a list of elements within this DEM
            # elementList = elementDict["DEM%s"%((len(demFilenameList)-1)-i)]
            elementList = elementDict["DEM%s"%demNumber]
            ...
            # for ele in range(2):
                ...
                print("Finished Element {0} of {1} in DEM {2} took {3}".format(countElementLoop,len(elementList),demNumber,end - start))
        ...
        for i in range(len(demFilenameList)):
            ...
            rasterFiles = [demFilenameList[i],landcoverFilenameList[i],i]
            future = e.submit(parallelDEM, rasterFiles) # parallelDEM(demAndLC)
            futures.append(future)
            
        results = [future.result() for future in futures]
        # for i in range(len(demFilenameList)):
            
        #     rasterFiles = [demFilenameList[i],landcoverFilenameList[i]]
        #     result = parallelDEM(rasterFiles)
        #     results.append(result)
  
        endtime = time.time()
        
        print('Took {} s'.format(endtime-starttime))
        
        return results
    
########################## PRACTICE PARALLEL ##################################

    # def parallelPractice(numberList):
    #     from concurrent.futures import ThreadPoolExecutor, ProcessPoolExecutor
    #     from dask.distributed import Client 
    #     import time

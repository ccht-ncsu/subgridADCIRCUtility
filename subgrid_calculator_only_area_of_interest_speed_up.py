    ...
    def calculateSubgridCorrection(controlFilename):
        ...
        for i in range(len(demFilenameList)):
            ...
            # for ele in elementList[:2]:
                ...
                currEleXCentroid = cp.mean(currXPerimeterPoints)
                currEleYCenteroid = cp.mean(currYPerimeterPoints)
                ...
                for j in range(3):
                    ...
                    for k in range(numSubElements):
                      ...
                      # finds rows and column to cut down arrays
                        ...
                        xCutGeoTiffMatrix2 = cp.asarray(xCutGeoTiffMatrix2)
                        yCutGeoTiffMatrix2 = cp.asarray(yCutGeoTiffMatrix2)
                        ...
                        calculationStartTime = time.perf_counter()
        ...
        ################ BEGIN VECTORIZED CALCULATIONS ##############################
                        ...
                        tempManningArray = nCutGeoTiffMatrix2[:,:,cp.newaxis]
                    
                        # subtract the bathymetry (2D Array) array from the surface 
                        # elevations (3D array) to get total water depths over the 
                        # sub element 
                    
                        tempTotWatDepthArray = cp.subtract(tempSurfaceElevArray,zCutGeoTiffMatrix2[:,:,None])
                        ...
                        calculationFirstHalfEndTime = time.perf_counter()
                        
                        
                    ################ CALCULATE WET FRACTION #######################
                        # wetFractionCalcStartTime = time.perf_counter()
                        # keep track of how many cells are wet for use in averaging 
                        # and wet fraction calulation
                    
                        wetDrySubElementList += tempCountWet
                        
                        # wetFractionCalcEndTime = time.perf_counter()
                        # print('wetFraction Took {} s'.format(wetFractionCalcEndTime-wetFractionCalcStartTime))
                    ...
                    #################### CALCULATING TOTAL WATER DEPTH ############
                        ...
                        # totWaterDepthStartTime = time.perf_counter()
                        ...
                        # totWaterDepthEndTime = time.perf_counter()
                        # print('totwaterDepth took {} s'.format(totWaterDepthEndTime-totWaterDepthStartTime))
                        #################### CALCULATING MANNINGS n ###################
                        # manningsStartTime = time.perf_counter()
                        ...
                        # manningsEndTime = time.perf_counter()
                        # print('mannings Calc Took {} s'.format(manningsEndTime-manningsStartTime))
                        ########### CALCULATE GRID AVERAGED CF FOR MANNING ############
                        
                        # calulate now for the subsubelement then sum later for use 
                        # in other calulations
                        # bottomFricStartTime = time.perf_counter()
                        # cfStartTime = time.perf_counter()
                        tempcf = 9.81*tempManningWetArray**2/tempTotWatDepthWetArray**(1/3)
                        # cfEndTime = time.perf_counter()
                        # print('CF took {} s'.format(cfEndTime-cfStartTime))
                        ###############################################################
                        ...
                        if level0andLevel1:
                            ...
                            # rvStartTime = time.perf_counter()
                            rvBottomTermList += cp.nansum(tempTotWatDepthWetArray**(3/2)*\
                                (tempcf)**(-1/2),axis = 0)
                            # rvEndTime = time.perf_counter()
                            # print('RV took {} s'.format(rvEndTime-rvStartTime))
                            ...
                            cadvMiddleTermList += cp.nansum(tempTotWatDepthWetArray**2/tempcf
                                                            ,axis=0)
                            # cadvEndTime = time.perf_counter()
                            # print('cadv took {} s'.format(cadvEndTime-cadvStartTime))
                        ...
                        # calculationSecondHalfEndTime = time.perf_counter()
                        
                        # print('Calulation First Half time = {} s'.format(calculationFirstHalfEndTime-calculationStartTime))
                        # print('Calulation Second Half time = {} s'.format(calculationSecondHalfEndTime-calculationFirstHalfEndTime))
                        
                        # calculationEndTime = time.perf_counter()
                        
                        # print('Calulation Time = {} s'.format(calculationEndTime - calculationStartTime))
                        
                        # subElementLoopEndTime = time.perf_counter()
                        
                        # print(subElementLoopEndTime - subElementLoopStartTime)
